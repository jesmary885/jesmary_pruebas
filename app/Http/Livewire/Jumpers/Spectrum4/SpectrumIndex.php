...
namespace App\Http\Livewire\Jumpers\Spectrum4;
...
class SpectrumIndex extends Component
{
    ...
    public function numerologia(){
        ...
        $this->emit('numerologia',$operacion_total,'jumpers.spectrum4.spectrum-index','verific');
    }
    ...
    public function verific($result){
        ...
        if($result[0] == $this->operacion->resultado){
            ...
            if($busqueda_trans != false){
                ...
                if($busq_elem1 > 50){
                    $elem1 = substr($this->search,($busqueda_trans + 14));
                }
                else{
                ...
                try {
                    ...
                    $resultado = $client->request('GET','/Spectrum/5/'.$elem1);
        
                    if($resultado->getStatusCode() == 200){

                        $link_register = new Links_usados();
                        $link_register->link = $this->search;
                        $link_register->k_detected  = 'SPECTRUM_JDPOWERONLINE';
    ...
    public function render()
    {
                    ...
                    if($busqueda_f_ != false){
                        ...
                        if($this->jumper_complete == "") {
                            $link_register_search = Links_usados::where('link',$this->search)
                                ->where('k_detected','SPECTRUM_JDPOWERONLINE')
                            ...
                            if($link_register_search){
                                ...
                                $links_usados = Links_usados::where('k_detected','SPECTRUM_JDPOWERONLINE')
                                    ->where('user_id',$this->user->id)
        ...
        return view('livewire.jumpers.spectrum4.spectrum-index');
    }
    ...
    public function clear(){
        ...
        session()->forget('search');
        $this->busqueda_link = "";
        return redirect()->route('spectrum4.index');
    }
}
