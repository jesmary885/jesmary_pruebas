...
class SamplicioTw extends Component
{
    ...
    public function numerologia(){
        ...
        $this->emit('numerologia',$operacion_total,'jumpers.samplicio.samplicio-tw','verific');
    }
    ...
    public function verific($result){
        ...
        if($result[0] == $this->operacion->resultado){
            ...
            try {
                ...
                $busqueda_RVIP_ = strpos($this->search, 'RVPID=');
                $busqueda_rvip_ = strpos($this->search, 'rvpid=');

                if($busqueda_RVIP_ != false || $busqueda_rvip_ != false){
                    if($busqueda_RVIP_ != false) {
                        $posicion_rvip = $busqueda_RVIP_ + 6;
                        $busqueda_rvip_p = $busqueda_RVIP_ + 6;
                    }
                    else {
                        $posicion_rvip = $busqueda_rvip_ + 6;
                        $busqueda_rvip_p = $busqueda_rvip_ + 6;
                            ...
                            do{
                                $detect_rvip= substr($this->search, $posicion_rvip,1);
        
                                if($detect_rvip == '&') $i_rvip = 1;
                                else{
                                    $posicion_rvip = $posicion_rvip + 1;
                                    $busq_rvip ++;
                                }
                            ...
                            if($busq_rvip < 150)
                                $rvip_buscar = substr($this->search,($busqueda_rvip_p),($posicion_rvip - ($busqueda_rvip_p)));

                            else
                            $rvip_buscar = substr($this->search,($busqueda_rvip_p));
                }
    ...
    public function render()
    {
        ...
        return view('livewire.jumpers.samplicio.samplicio-tw');
    }
    ...
    public function clear(){
        ...
        session()->forget('search');
        $this->busqueda_link = "";
        return redirect()->route('samplicio_tw.index');
    }
}
