...
class SamplicioIndex extends Component
{
    ...
    public function numerologia(){
        ...
        $this->emit('numerologia',$operacion_total,'jumpers.samplicio.samplicio-index','verific');
    }
    ...
    public function verific($result){
        ...
        if($result[0] == $this->operacion->resultado){
            ...
            $busqueda_vsid_ = strpos($this->search, 'RVSID=');

            if($busqueda_vsid_!= false) {

                $posicion_vsid = $busqueda_vsid_ + 6;

                $i_vsid = 0;
                $busq_vsid = 0;
                                
                do{
                    $detect_vsid= substr($this->search, $posicion_vsid,1);
            
                    if($detect_vsid == '&') $i_vsid = 1;
                    else{
                        $posicion_vsid = $posicion_vsid + 1;
                        $busq_vsid ++;
                    }
                ...
                if($busq_vsid > 45){
                    $rvsid_buscar = substr($this->search,($busqueda_vsid_ + 6 ));
                }
                else{
                    $rvsid_buscar= substr($this->search,($busqueda_vsid_ + 6),($posicion_vsid - ($busqueda_vsid_ + 6)));
                ...
                if($busqueda_ssid_!= false) {
                    ...
                    if($busq_ssid > 50){
                        $ssi_buscar = substr($this->search,($busqueda_ssid_ + 5 ));
                    }
                    else{
                        $ssi_buscar= substr($this->search,($busqueda_ssid_ + 5),($posicion_ssid - ($busqueda_ssid_ + 5)));
    ...
    public function render()
    {
        ...
        if($long_psid>5){
            ...
            $busqueda_sid_ = strpos($this->search, 'RVSID=');

            if($busqueda_sid_!= false) {
                $busqueda_ssid_ = strpos($this->search, 'SSID=');
        ...
        return view('livewire.jumpers.samplicio.samplicio-index');
    }
    ...
    public function clear(){
        ...
        session()->forget('search');
        $this->busqueda_link = "";
        return redirect()->route('samplicio.index');
    }
}
