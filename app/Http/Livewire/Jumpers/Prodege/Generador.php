...
namespace App\Http\Livewire\Jumpers\Prodege;
...
class Generador extends Component
{
    ...
    public function numerologia(){
        ...
        $this->emit('numerologia',$operacion_total,'jumpers.prodege.generador','verific');
    }
    ...
    public function verific($result){
        ...
        if($result[0] == $this->operacion->resultado){
            ...
            try {
                ...
                $resultado = $client->request('GET', '/Prodege/1/'.$this->search);
    
                if($resultado->getStatusCode() == 200){

                    $link_register = new Links_usados();
                    $link_register->link = $this->search;
                    $link_register->k_detected  = 'PRODEGE';
    ...
    public function render()
    {
        ...
        if($long_psid>5){
            ...
            if($this->jumper_complete == "" && $this->jumper_detect == 0) {
                ...
                if($link_register_search){
                    ...
                    $links_usados = Links_usados::where('k_detected','PRODEGE')
                        ->where('user_id',$this->user->id)
        ...
        return view('livewire.jumpers.prodege.generador');
    }
    ...
    public function clear(){
        ...
        session()->forget('search');
        $this->busqueda_link = "";
        return redirect()->route('prodege.generador');
    }
