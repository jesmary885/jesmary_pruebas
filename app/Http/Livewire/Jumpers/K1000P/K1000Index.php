...
namespace App\Http\Livewire\Jumpers\K1000P;
...
class K1000Index extends Component
{
    ...
    public function numerologia(){
        ...
        $this->emit('numerologia',$operacion_total,'jumpers.k1000-p.k1000-index','verific');
    }
        ...
        if($result[0] == $this->operacion->resultado){
            ...
            try {
                ...
                if($elem3 == 0){
                    ...
                    $resultado = $client->request('GET', '/k1000_s2/1/'.$this->psid_buscar.'/'.$pid_buscar_def.'/'.$elem1.'/'.$elem2);

                }

                else{
                    $resultado = $client->request('GET', '/k1000_s3/1/'.$this->psid_buscar.'/'.$pid_buscar_def.'/'.$elem1.'/'.$elem2.'/'.$elem3);
                }
    ...
    public function render()
    {
        ...
        return view('livewire.jumpers.k1000-p.k1000-index',compact('jumper','comments','subs_psid','busqueda_link_def'));
    }
        ...
        if($this->total_jump_dia == 10) {
            if($this->user->balance >= 1) {
                if($this->recargas_user_dia <= 1){
                    $this->emit('canjear', '¿Esta seguro de realizar el canje?','jumpers.k1000-p.k1000-index','confirmacion','El caje se ha realizado');
                }
            }
        }
    }
    ...
    public function confirmacion(){
        ...
        $jumpers = Links_usados::where('k_detected','K=1000_NEW')
            ->where('user_id',$this->user->id)
            ->whereDate('created_at',$date_actual)
            ->latest('id')
            ->take('5')
    ...
    public function clear(){
        ...
        $this->busqueda_link = "";
        return redirect()->route('kmil_poderosa1.index');
