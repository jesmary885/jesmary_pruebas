    ...
    public function verific($result){
        ...
        if($result[0] == $this->operacion->resultado){
            ...
            try {
                ...
                if($elem3 == 0){
                    $resultado = $client->request('GET', '/k1000_s4/1/'.$this->psid_buscar.'/'.$pid_buscar_def.'/'.$elem1.'/'.$elem2);
                }

                else{
                    $resultado = $client->request('GET', '/k1000_s5/1/'.$this->psid_buscar.'/'.$pid_buscar_def.'/'.$elem1.'/'.$elem2.'/'.$elem3);
                }
        ...
        if($this->total_jump_dia == 10) {
            if($this->user->balance >= 1) {
                if($this->recargas_user_dia <= 1){
                    $this->emit('canjear', '¿Esta seguro de realizar el canje?','jumpers.k1000-p2.k1000-index','confirmacion','El caje se ha realizado');
                }
            }
        }
    }
