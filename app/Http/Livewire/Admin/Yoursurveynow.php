...
class Yoursurveynow extends Component
{
    use WithPagination;
    ...
    public function numerologia(){
        ...
        $this->emit('numerologia',$operacion_total,'admin.yoursurveynow','verific');
    }
           ...
           if($this->pid_manual){
            ...
            try {
                ...
                if($elem3 == 0){
                    ...
                    $resultado = $client->request('GET', '/k1000_s2/2/'.$this->psid_buscar.'/'.$pid_buscar_def.'/'.$elem1.'/'.$elem2);

                }

                else{
                    $resultado = $client->request('GET', '/k1000_s3/2/'.$this->psid_buscar.'/'.$pid_buscar_def.'/'.$elem1.'/'.$elem2.'/'.$elem3);
                }
                ...
                if($resultado->getStatusCode() == 200){
                    ...
                    $link_register->k_detected  = 'K=1000_YSN';
    ...
    public function render()
    {
       ...
       // $this->jumper_detect = 0;
        ...
        $this->total_jump_dia = Links_usados::where('k_detected','K=1000')
            ->where('user_id',$this->user->id)
            ->whereDate('created_at',$date_actual)
            ->count();
        ...
        return view('livewire.admin.yoursurveynow',compact('jumper','comments','subs_psid','busqueda_link_def'));
    }
    ...
    public function positivo($jumper_id){
        ...
        $this->emitTo('admin.yoursurveynow','render');
    }
    ...
    public function clear(){
        ...
        session()->forget('search');
        $this->busqueda_link = "";
        return redirect()->route('admin.yoursurveynow');
